...
//amaç sadece veriyi değiştirmekse bu şekilde yapabiliriz. Gerçek hayattaki projelerde yaptığımız da budur.
//Person? person = await context.Persons.FirstOrDefaultAsync(p => p.Id == 1);
//person.Address = new() { PersonAddress = "Yeni Adres" }; 

//amaç ilişkilere de müdahale etmekse- önce ilişkiyi silip sonra yeni bir adres ekleyeceğiz
...
////Person? person = await context.Persons.FindAsync(2); //adrese bağlamak için mevcut person ı elde etme
////address.Person = person;

//address.Person = new() //adrese bağlanacak yeni person oluşturma
//{
//    Name = "Rıfkı"
//};
...
//Post? silinecekPost = context.Posts.FirstOrDefault(p => p.Id == 2);
