...
//Bu davranış modelleri Fluent API ile konfigüre edilebilmektedir. Yukarıda one to one ve one to many de incelediğimiz durumlarda principal entitye bağlı olan dependent entity verilerini silmiştik. Bu davranışlar ise  direkt principal entityden veri sildiğimizde kullandığımız davranışlardır.
#region Cascade
//Esas tablodan silinen veriyle karşı/bağımlı tabloda bulunan ilişkili verilerin silinmesini sağlar.
//not: n to n ilişkililerde yalnızca bu davranışa izin verilir.
#endregion

#region SetNull
//Esas tablodan silinen veriyle karşı/bağımlı tabloda bulunan ilişkili verilere null değerin atanmasını sağlar.

//One to One senaryolarda eğer ki Foreign key ve Primary key kolonları aynı ise o zaman SetNull davranuışını KULLANAMAYIZ! kullanabilmek için foreign key kolonunu ayrı tanımlamalıyız. ve foreihn key kolonunu nullable yapmalı, ve fluent api de isrequired(false) olarak ayarlamalıyız.
#endregion
